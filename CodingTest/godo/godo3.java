...
가장 적게 등장하는 글자를 c2라 한다 (c2는 반드시 한번 이상은 등장해야 한다). s의 거친 정도는 c1이 등장하는 횟수에서 c2가 등장하는 횟수를 뺀 숫자이다.

당신은 주어진 문자열 s에서 0개 이상, int n개 이하의 글자를 삭제하는 방법으로 문자열을 변경할 수 있다 (명확한 이해를 위해 예제를 참조하라).
이 방법을 이용하여 얻을 수 있는 가장 적은 문자의 거친 정도를 반환하여라.
